...
export const addUserDataController = async (request, response) => {
  ...
  if (!name) {
    return response.status(400).send({message:"User name is not found",error:true});
  } else if (!city) {
    return response.status(400).send({message:"city is not found",error:true});
  } else if (!education) {
    return response.status(400).send({message:"education is not found",error:true});
  } else if (!email) {
    return response.status(400).send({message:"email is not found",error:true});
  } else if (!password) {
    return response.status(400).send({message:"password is not found",error:true});
  }
...
export const loginUserController = async (request,response) => {
  ...
  if (!email) {
    return response.status(400).send({error:true,message:"Email is not found"});
  } else if (!password) {
    return response.status(400).send({error:true,message:"password is not found"});
  }
